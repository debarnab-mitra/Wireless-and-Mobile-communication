...
plot2d(M,SINR_avg_all_messages,color("blue"),logflag = "nl");
  ...
  //foreach M station, generate a random BPSK message sequence and a wideband random sequence
    ...
    while j <= message_size //message_index
     ...
     while k <= M(user_index)// user index
        ...
        SINR_avg(j) = sum(SINR)/M(user_index);
     j = j+1
    end
    
    SINR_avg_all_messages(user_index) = sum(SINR_avg)/message_size;
    Pe(user_index) = num_error/(message_size*(M(user_index)))
  
    user_index = user_index+1;
end

plot2d(M,SINR_avg_all_messages,color("red"),logflag = "nl");


xtitle( 'SINR vs No. of MSs (M)', 'No. of MSs (M)', 'SINR');
legend(["Single Cell","Multi-cell"]);

...
while i <=1000
    ...
    CDF3(i) = (1/(j-1))*(nnz(dsearch(SIR_dB, [0,p(i)])))
    i = i+1
end
...
while i <=1000
    ...
    CDF2(i) = (1/(j-1))*(nnz(dsearch(SIR_dB, [0,p(i)])))
    i = i+1
end
...
v= [2,1,0,-1,2,1,0];
//coordinates of ref cell
x_ref = [1/2,1,1/2,0]*(Rc);
y_ref = [(sqrt(3)/2),0,-(sqrt(3)/2),0]*(Rc);
//plot(x_ref,y_ref,'o');
...
//take only those poins which lie within a sector
while i <= 5000
...
//foreach point calculate the SIR values
while i <= j-1
...
//intereference distances
    ...
    dist_5 = sqrt((required_points_x(i) - x_intf(5))^2 + (required_points_y(i) - y_intf(5))^2);
    dist_6 = sqrt((required_points_x(i) - x_intf(6))^2 + (required_points_y(i) - y_intf(6))^2);
    dist_7 = sqrt((required_points_x(i) - x_intf(7))^2 + (required_points_y(i) - y_intf(7))^2);

//calculate SIR
    SIR(i) = ((1/(dist))^(gamma1))/((1/(dist_1))^(gamma1)+(1/(dist_2))^(gamma1)+(1/(dist_3))^(gamma1)+(1/(dist_4))^(gamma1)+(1/(dist_5))^(gamma1)+(1/(dist_6))^(gamma1)+ (1/(dist_7))^(gamma1));
    i = i+1;
end
// calculate the CDF of the SIR taken in dB scale
SIR_dB = 10*log10(SIR);
...
clf(2); 
plot(p,CDF3,'r');
xtitle( 'CDF for reuse 3', 'SIR', 'CDF')

plot(p,CDF2,'g');
xtitle( 'CDF for reuse 7', 'SIR', 'CDF')

plot(p,CDF,'b');
xtitle( 'Comparing CDF in the three cases', 'SIR', 'CDF');
legend(["Reuse 3", "Reuse 7","Reuse 1 with sectoring"]);

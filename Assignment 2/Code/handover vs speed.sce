...
num_handouts = [0,0,0]; // keeping the handover counts
// Direction theta (in degrees)
theta = grand(1, 1, "unf", 0, 360);


////////Start if main loop of velocity
while z >=1
    ...
    while dist <=5000
        ...
        if (current_BS ~= index & (RSS_self + H < val))
          ...
          // plot(x(i),y(i),'o');
           m = m +1;
           num_handouts(z) = num_handouts(z) + 1;
        end    
...
scf(2);
clf(2);
plot(v*18/5,num_handouts);
xtitle( ' No. of Handovers vs Speed', 'speed (in km/hr)', 'No. of handovers')

...
//plots the different cells 
k = 2;
while k <=19
    plot(x_ref1 + x_neigh(k),y_ref1+ y_neigh(k));
    k = k+1;
end
...
x_handover = zeros(200,1);// keeps track of the handover coordinates
...
j = 1;
theta_initial = theta;
current_BS = 1;//keeps track of the current BS
m = 1; n = 1; p = 1;
RSS_sum = zeros(1,19);
random = grand(1,19, "exp",1);
dist = 0;

//start of movement loop
while num_handouts <=200
    
    x(i) = x(i-1) + v*t_step*cos(theta*%pi/180);
    y(i) = y(i-1) + v*t_step*sin(theta*%pi/180);
    dist = dist +  v*t_step;
    //plot(x(i),y(i),'ro-');
    ...
    //Handover LOGIC
    if (current_BS ~= index & (RSS_self + H < val))
       current_BS = index;
       x_handover(m) = x(i);
       y_handover(m) = y(i);
      // plot(x(i),y(i),'o');
...
l = nnz(x);
plot(x(1:l),y(1:l),'ro-');
plot(x_handover,y_handover,'o');
plot(required_points_x(1),required_points_y(1),'gs-');
//legend(["start point","trajectory", "Handover Instances"]);
